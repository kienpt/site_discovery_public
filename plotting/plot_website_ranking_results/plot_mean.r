...
mm <- max(data)
s <- 1.5
xx <- barplot(as.numeric(data[1,]), main=args[3], xlab="Mean", horiz=TRUE, names.arg=colnames(data), las=1, font.lab=1, font.main=1, family='serif', xlim=c(0, mm*1.1), cex.names=s, cex.axis=s, cex.lab=2.5, cex.main=2.5, cex.sub=s) #font.lab and font.main defines font size

text(y = xx-0.3, x = as.numeric(data[1,]) + 0.09*mm, label = as.numeric(data[1,]), pos = 3, cex = s, family="serif") # show text on top of each bar
